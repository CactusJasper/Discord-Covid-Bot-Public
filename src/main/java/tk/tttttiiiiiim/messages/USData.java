...
public class USData
{
    public static String getUSDataMessage()
    {
        try
        {
            URL url = new URL("https://corona.lmao.ninja/v2/countries/US");
            ...
            String text = "== US SARS-COV-2 Stats ==\n";
            ...
            text += "Today's Recovered: " + data.todayRecovered + "\n";
            ...
            text += "One Test every " + data.oneTestPerPeople + " people" + "\n";
    ...
    public static ArrayList<String> getUSExtraDataMessage()
    {
        try
        {
            URL url = new URL("https://corona.lmao.ninja/v2/states");
            HttpURLConnection connection = (HttpURLConnection) url.openConnection();
            connection.setRequestMethod("GET");
            connection.setRequestProperty("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/86.0.4240.111 Safari/537.36");
            ...
            Gson gson = new Gson();
            StateData[] data = gson.fromJson(content.toString(), new TypeToken<StateData[]>(){}.getType());
            ArrayList<String> messages = new ArrayList<>();

            String text = "== US Extra Data ==\n";
            for(int i = 0; i < data.length; i++)
            {
                text += data[i].state + ": \n";
                text += "    Cases: " + data[i].cases + "\n";
                text += "    Deaths: " + data[i].deaths + "\n";
                text += "    Tests: " + data[i].tests + "\n";
                text += "    Population: " + data[i].population + "\n";

                if(i == 15 || i == 25 || i == 35 || i == 45 || i == 55 || i == data.length - 1)
                {
                    messages.add(text);
                    text = "";
                }
